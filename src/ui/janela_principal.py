...
class JanelaPrincipal(ctk.CTk):
    ...
    def mostrar_tela_pacientes(self):
        self._limpar_conteudo_principal()
        lbl = ctk.CTkLabel(self.main_frame,
                           text='Tela de Pacientes (Em desenvolvimento)', 
    ...
    def _criar_sidebar(self):
        ...
        self.btn_novo_atendimento = self._gerar_botao_menu('Novo Atendimento', icone='plus-circle')
    ...
    def _desenhar_dashboard(self):
        # Título
        self.lbl_titulo = ctk.CTkLabel(self.main_frame, text='Resumo Geral', 
        ...
        self._cria_card('Total de Pacientes', str(stats['total_pacientes']), config.COR_PRINCIPAL)
        self._cria_card('Atendimentos Hoje', str(stats['atendimentos_hoje']), config.COR_SUCESSO)
        self._cria_card('Total Atendimentos', str(stats['total_atendimentos']), config.COR_AVISO)

        # Tabela
        self._criar_tabela_atendimentos(stats['total_atendimentos'])

    def _cria_card(self, titulo, valor, cor_destaque):
        card = ctk.CTkFrame(self.cards_container, fg_color=config.COR_CARD, width=250, height=150, corner_radius=15)
        ...
        ctk.CTkLabel(card, text=titulo.upper(), font=(config.FONTE_FAMILIA, 12, 'bold'), text_color='gray').pack(pady=(20, 0), padx=20, anchor='w')
        ctk.CTkLabel(card, text=valor, font=config.FONTE_CARD_VALOR, text_color=config.COR_TEXTO_DARK).pack(pady=(5, 0), padx=20, anchor='w')

    def _criar_tabela_atendimentos(self, total_atendimentos):
        self.tabela_frame = ctk.CTkFrame(self.main_frame, fg_color=config.COR_CARD, corner_radius=15)
        self.tabela_frame.pack(fill='x', pady=(40, 0))

        ctk.CTkLabel(self.tabela_frame, text='Últimos Atendimentos', font=config.FONTE_H2, text_color=config.COR_TEXTO_DARK).pack(pady=20, padx=20, anchor='w')
        ...
        for i, header in enumerate(headers):
            ctk.CTkLabel(self.grid_frame, text=header, font=(config.FONTE_FAMILIA, 13, 'bold'), text_color='gray', anchor='w').grid(row=0, column=i, sticky='ew', padx=10, pady=10)
    ...
    def _criar_atendimento_row(self, data, row_idx):
        for col_idx, value in enumerate(data):
            if col_idx == 3:
                ...
                ctk.CTkLabel(self.grid_frame, text=value, font=config.FONTE_CORPO, text_color=config.COR_TEXTO_DARK, anchor='w').grid(row=row_idx, column=col_idx, sticky='ew', padx=10, pady=5)

    def _criar_status_pill(self, status, row_idx):
        bg_cor = config.COR_STATUS_REALIZADO if status == 'Realizado' else config.COR_STATUS_ACOMPANHAMENTO
        txt_cor = config.COR_STATUS_REALIZADO_TEXT if status == 'Realizado' else config.COR_STATUS_ACOMPANHAMENTO_TEXT

        pill = ctk.CTkFrame(self.grid_frame, fg_color=bg_cor, corner_radius=20, height=30)
        pill.grid(row=row_idx, column=3, sticky='w', padx=10, pady=5)
        pill.grid_propagate(False)
        pill.pack_propagate(False)

        ctk.CTkLabel(pill, text=status, font=config.FONTE_PILL, text_color=txt_cor).pack(pady=0, padx=20, fill='both')